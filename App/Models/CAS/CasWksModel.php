...
use App\Core\GenerateKey;
use App\Metadata\CAS\CasWksMD;
use App\Models\Traits\CrudOperationsTrait;
use App\Traits\ReferencedTables;

class CasWksModel extends CasWksMD
{
    ...
    {
        if (empty($this->att['CasWksBlq'])) {
            $this->att['CasWksBlq'] = 'N';
        }
        if ($this->att['CasWksBlq'] == 'N') {
            $this->att['CasWksBlqDtt'] = null;
        }
        if ($this->att['CasWksBlq'] == 'S' && empty($this->att['CasWksBlqDtt'])) {
            $this->att['CasWksBlqDtt'] = $dtnow_str;
        }
        if (empty($this->att['CasWksMac'])) {
            $this->att['CasWksMac'] = '00:00:00:00:00:00';
        }
        if (empty($this->att['CasWksEip'])) {
            $this->att['CasWksEip'] = '0.0.0.0';
        }
        if (empty($this->att['CasWksChv'])) {
            $this->att['CasWksChv'] = GenerateKey::generateKey();
        }
    }
}
